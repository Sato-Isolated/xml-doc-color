...
export function parseDocLine(content: string): ParsedToken[] {
    ...
    while (i < n) {
        if (content[i] === '<') {
            ...
            textStart = i;
            // If parseXmlTag returned early (e.g. unterminated <!--), i === before would loop; guard:
            if (i === before) { i++; textStart = i; }
...
function parseXmlTag(content: string, i: number, n: number, tokens: ParsedToken[]): number {
    ...
    if (i + 1 < n && content[i + 1] === '/') {
        tokens.push({ type: TOKEN_TYPES.xmlDocTagDelimiter, start: i, length: 2 });
        i += 2;
        const nameStart = i;
        while (i < n && /[\w:.-]/.test(content[i])) { i++; }
        if (i > nameStart) {
            tokens.push({ type: TOKEN_TYPES.xmlDocTagName, start: nameStart, length: i - nameStart });
        }
        while (i < n && content[i] === ' ') { i++; }
        if (i < n && content[i] === '>') {
            tokens.push({ type: TOKEN_TYPES.xmlDocTagDelimiter, start: i, length: 1 });
            i++;
        }
        return i;
    }
    ...
    while (i < n) {
        ...
        while (i < n && /[\w:.-]/.test(content[i])) { i++; }
        if (i > attrStart) {
            tokens.push({ type: TOKEN_TYPES.xmlDocAttribute, start: attrStart, length: i - attrStart });
        } else {
        ...
        if (i < n && content[i] === '=') {
            ...
            if (i < n && (content[i] === '"' || content[i] === "'")) {
                ...
                tokens.push({ type: TOKEN_TYPES.xmlDocAttributeValue, start: valStart, length: i - valStart });
            }
        }
    }
